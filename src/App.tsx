...
function MemberReport() {
  ...
  const memberSavingRows = (() => {
    ...
    memberMonths.forEach((m, i) => {
      ...
      let r = a || d || f;
      if (r) {
        const startCapital = hasStarted ? prevTotal : num(r.startCapital);
        const addSaving = num(r.addSaving);
        const withdraw = num(r.withdraw);
        const total = startCapital + addSaving - withdraw;
        
        r = { ...r, startCapital: startCapital.toFixed(2), total: total.toFixed(2) };
        prevTotal = total;
        hasStarted = true;

        const row: any = { seq: String(i + 1).padStart(2, '0'), mi: i, monthName: m.split(' ')[0], ...r };
        if (f) row.profit = f.interest;
        out.push(row);
      }
    });
    return out;
  ...
  const memberSavingsTotal = memberSavingRows.length ? num(memberSavingRows[memberSavingRows.length - 1].total) : 0;
